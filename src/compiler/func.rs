...
impl Func {
    ...
    pub(super) fn bind_partial(self, compiler: &mut Compiler, positional_args: &[Value], named_args: &ValueMap, content_arg: Value) -> Result<Func, errors::PapyriError> {
        let mut binder = self.get_partials()
            .open(compiler, self.signature());
    ...
    pub(super) fn bind_synthetic_call(&self, compiler: &mut Compiler, bind_implicits: bool, content_value: Value) -> Result<ValueMap, errors::PapyriError> {
        let mut binder = self.get_partials()
            .open(compiler, self.signature());
        if bind_implicits {
    ...
    fn bind_call(&self, compiler: &mut Compiler, call: &ast::FuncCall) -> Result<ValueMap, errors::AlreadyReported> {
        let mut binder = self.get_partials()
            .open(compiler, self.signature());
        ...
        binder.bind_implicit_args()
            .and_then(|_| binder.build())
            .map_err(|e| compiler.report(e, call.range))
    }
}
...
impl <'a> Compiler<'a> {
    ...
    pub(super) fn evaluate_func_call_with_bindings(&mut self, func: Func, bindings: ValueMap, type_hint: &Type, call_range: SourceRange) -> Result<Value, errors::AlreadyReported> {
        match func {
