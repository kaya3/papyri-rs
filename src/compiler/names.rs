    ...
    fn evaluate_attr(&mut self, attr: &ast::AttrName) -> Option<Value> {
        ...
        match &subject {
            ...
            Value::Int(i) => match attr_id {
                str_ids::ADD => return self.bind_pos_arg(natives.INT.ADD.clone(), subject, attr.range),
                str_ids::NEGATE => return Some((-i).into()),
                _ => {},
            },
            
            Value::Str(s) => match attr_id {
                str_ids::ESCAPE_HTML => return self.bind_method(natives.HTML.ESCAPE_HTML.clone(), subject, attr.range),
                ...
                str_ids::TRIM => return Some(s.trim().into()),
                _ => {},
            },
            ...
            Value::List(vs) => match attr_id {
                str_ids::ALL => return self.bind_method(natives.LIST.ALL.clone(), subject, attr.range),
                str_ids::ANY => return self.bind_method(natives.LIST.ANY.clone(), subject, attr.range),
                str_ids::CONTAINS => return self.bind_method(natives.LIST.CONTAINS.clone(), subject, attr.range),
                str_ids::FILTER => return self.bind_method(natives.LIST.FILTER.clone(), subject, attr.range),
                str_ids::FIND => return self.bind_method(natives.LIST.FIND.clone(), subject, attr.range),
                str_ids::FLAT => return Some(Value::flatten_list(vs.as_ref()).into()),
                str_ids::IS_EMPTY => return Some(vs.is_empty().into()),
                str_ids::JOIN => return self.bind_method(natives.LIST.JOIN.clone(), subject, attr.range),
                str_ids::LEN => return Some(Value::Int(vs.len() as i64)),
                str_ids::MAP => return self.bind_method(natives.LIST.MAP.clone(), subject, attr.range),
                str_ids::REVERSED => return Some(Value::reverse_list(vs.as_ref()).into()),
                str_ids::SLICE => return self.bind_method(natives.LIST.SLICE.clone(), subject, attr.range),
                str_ids::SORTED => return self.bind_method(natives.LIST.SORTED.clone(), subject, attr.range),
                _ => {},
            },
            
            Value::Func(f) => match attr_id {
                str_ids::BIND => return self.bind_pos_arg(natives.FUNCTION.BIND.clone(), f.clone().into(), attr.range),
                str_ids::NAME => return Some(self.get_name(f.name_id()).into()),
                _ => {},
            },
            
            Value::Regex(_) => match attr_id {
                str_ids::FIND => return self.bind_pos_arg(natives.REGEX.FIND.clone(), subject, attr.range),
                str_ids::FIND_ALL => return self.bind_pos_arg(natives.REGEX.FIND_ALL.clone(), subject, attr.range),
                str_ids::TEST => return self.bind_pos_arg(natives.REGEX.TEST.clone(), subject, attr.range),
                _ => {},
            },
